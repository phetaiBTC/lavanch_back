...
export interface IMemberPointRepository {
  ...
  findByMemberAndBranch(memberId: number, branchId: number): Promise<MemberPoint | null>;
  create(memberPoint: MemberPoint): Promise<MemberPoint>;
  addPoints(memberId: number, branchId: number, points: number): Promise<MemberPoint>;
  subtractPoints(memberId: number, branchId: number, points: number): Promise<MemberPoint>;
