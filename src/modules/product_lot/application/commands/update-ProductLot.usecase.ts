...
export class UpdateProductLotUseCase {
  ...
  async execute(id: number, dto: UpdateProductLotDto): Promise<ProductLot> {
    ...
    if (dto.manufacture_date !== undefined)
      entity.manufacture_date = new Date(dto.manufacture_date);
    if (dto.expiry_date !== undefined)
      entity.expiry_date = new Date(dto.expiry_date);
    if (dto.quantity !== undefined) entity.quantity = dto.quantity;
    if (dto.cost_price_local !== undefined)
      entity.cost_price_local = dto.cost_price_local;
    if (dto.cost_price_original !== undefined)
      entity.cost_price_original = dto.cost_price_original;
